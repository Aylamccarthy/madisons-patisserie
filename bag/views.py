    ...
    def post(self, request, product_id):
        ...
        try:
            ...
            if quantity in range(1, product.stock + 1):
                ...
                messages.error(
                    request,
                    f"Quantity input value for <b>{product.name}</b>\
                        cannot be negative",
                    extra_tags="safe",
                )
            else:
                messages.error(
                    request,
                    f"The quantity chosen for <b>{product.name}</b>\
            ...
            messages.error(
                request,
                f"Quantity input type for <b>{product.name}</b>\
    ...
    def delete(self, request, product_id):
        ...
        try:
            ...
            if str(product_id) in list(bag.keys()):
                ...
                messages.success(
                    request,
                    f"<b>{product.name}</b> was removed from\
                     your shopping bag",
                    extra_tags="safe",
                )
            else:
                messages.error(
                    request,
                    f"<b>{product.name}</b> was not found in the\
    ...
    def post(self, request, product_id):
        ...
        try:
            ...
            if quantity in range(1, product.stock + 1):
                ...
                messages.success(
                    request,
                    f"The quantity for <b>{product.name}</b>\
                     was updated to {bag[str(product_id)]}",
                    extra_tags="safe",
                )
                request.session["bag"] = bag
            elif quantity < 0:
